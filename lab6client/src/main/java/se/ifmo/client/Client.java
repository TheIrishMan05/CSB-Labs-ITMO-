...
public class Client implements AutoCloseable {
    ...
    public void start() throws IOException, ClassNotFoundException {
        ...
        main: while (handle && (!requests.isEmpty() || (input = console.writeAndRead("$ ")) != null && !input.equals("exit"))) {
            ...
            if (!requests.isEmpty()) input = requests.pollFirst();
            ...
            while (requiredElements-- > 0) {
                ...
                if (requests.isEmpty()) objectInput = ElementManager.collect(console);
                else objectInput = ElementManager.collect(requests);
